...
import { SEGMENT_BAND_ADJUSTMENT, DESK_BAND_ROLE, ASM_FLOW_CONSTRAINT } from '../config/thresholds.mjs';
 ...
 *
 * It NEVER changes the verdict (that is size and float, which ASM does not
 * touch). It is `binding` only where a trade is actually implied — a `stable`
 * company under ASM has no flow to constrain, so the stage is recorded but
 * nothing is qualified. Returns null when the company is not under ASM or the
 * qualifier is switched off.
 */
