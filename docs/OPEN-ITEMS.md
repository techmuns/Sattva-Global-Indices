...
| **The Yahoo benchmark fetcher's shrink guard is a hard stop on a soft step** | small | On 31 Aug and 1 Sep the benchmarks step failed with *"Refusing to write — dates we already held did not come back: EEM: 1 missing — 2026-08-28"*, so those two builds ran on a stale benchmark file. The step is soft, so the job continued; verify-data 55 now catches the resulting state, but the fetcher's own guard is zero-tolerance against a provider that reissues history. |
